...
class JPMathDataset(data.Dataset):
    """
    JPMathDatasetのDatasetを作成するクラス。PyTorchのDatasetクラスを継承。
...
def make_loc_conf(num_classes=3, bbox_aspect_num=[4, 6, 6, 6, 4, 4]):   ######################################## 変更 ######################################## num_classes = 21 ⇒ 3
...
#class Detect():     # 変更　https://github.com/YutaroOgawa/pytorch_advanced/issues/71
...
#        self.nms_thresh = nms_thresh  # nm_supressionでIOUがnms_thresh=0.45より大きいと、同一物体へのBBoxとみなす
    ...
    def forward(ctx, loc_data, conf_data, dbox_list):
        ...
        num_classes = conf_data.size(2)  # クラス数 = 3

        # confはソフトマックスを適用して正規化する
        #conf_data = self.softmax(conf_data)
        conf_data = ctx.softmax(conf_data)

        # 出力の型を作成する。テンソルサイズは[minibatch数, 3, 200, 5]
        ...
        return output  # torch.Size([1, 3, 200, 5])
...
class SSD(nn.Module):
    ...
    def __init__(self, phase, cfg):
        ...
        self.num_classes = cfg["num_classes"]  # クラス数=3
        ...
        if phase == 'inference':
            self.detect = Detect.apply      # 変更　https://github.com/YutaroOgawa/pytorch_advanced/issues/71
    ...
    def forward(self, x):
        ...
        if self.phase == "inference":  # 推論時     変更　https://github.com/YutaroOgawa/pytorch_advanced/issues/71
            # クラス「Detect」のforwardを実行
            # 返り値のサイズは torch.Size([batch_num, 3, 200, 5])
